...
def tok(ex, en_nlp, de_nlp, max_length):
    ...
    return {'src_text': ex['en'], 'trg_text': ex['de'], 'src_tokens': [SOS, *en, EOS], 'trg_tokens': [SOS, *de, EOS]}
...
def tok_local(ex, src_col, tgt_col, max_length):
    ...
    return {'src_text': ex[src_col], 'trg_text': ex[tgt_col], 'src_tokens': [SOS, *src, EOS], 'trg_tokens': [SOS, *tgt, EOS]}
...
def split_local_csv(path, src_col, tgt_col, valid_ratio, test_ratio, seed, max_train, max_valid, max_test):
    ...
    df = df[(df[src_col] != '') & (df[tgt_col] != '')].sample(frac=1, random_state=seed).reset_index(drop=True)
    n = len(df)
    if n < 3:
        raise ValueError('local CSV needs at least 3 parallel sentence pairs for train/valid/test split.')
...
def num(ds, sv, tv):
    out = []
    for item in ds:
        out.append({'src_text': item.get('src_text', item.get('zh', item.get('en', ''))), 'trg_text': item.get('trg_text', item.get('en', item.get('de', ''))), 'src_tokens': item['src_tokens'], 'trg_tokens': item['trg_tokens'],
...
def translate_set(model, rows, tv, device, max_len):
    ...
    for item in tqdm(rows, desc='test', leave=False):
        ...
        details.append({'source': item.get('src_text', item.get('zh', item.get('en', ''))), 'reference': item.get('trg_text', item.get('en', item.get('de', ''))), 'prediction': ' '.join(ptok), 'reference_tokens': rtok, 'predicted_tokens': ptok, 'attention': att.tolist(), 'source_tokens': item['src_tokens']})
...
def main():
    ...
    ap.add_argument('--local-csv', type=str, default=None, help='Path to local zh-en parallel CSV. Default columns: zh,en.')
    ap.add_argument('--src-col', type=str, default='zh', help='Source column name in local CSV.')
    ap.add_argument('--tgt-col', type=str, default='en', help='Target column name in local CSV.')
    ap.add_argument('--valid-ratio', type=float, default=0.1, help='Validation split ratio for local CSV.')
    ap.add_argument('--test-ratio', type=float, default=0.1, help='Test split ratio for local CSV.')
    ap.add_argument('--output-dir', type=str, default=None, help='Output directory. Defaults to outputs or outputs_zh_en.')
    args = ap.parse_args()

    os.environ.setdefault('HF_ENDPOINT', HF_MIRROR); set_seed(args.seed)
    base = Path(__file__).resolve().parent
    default_out = 'outputs_zh_en' if args.local_csv else 'outputs'
    outdir = Path(args.output_dir) if args.output_dir else base / default_out
    outdir.mkdir(exist_ok=True)
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu'); print(f'Using device: {device}')

    if args.local_csv:
        train, valid, test = split_local_csv(args.local_csv, args.src_col, args.tgt_col, args.valid_ratio, args.test_ratio, args.seed, args.max_train_samples, args.max_valid_samples, args.max_test_samples)
        kw = {'src_col': args.src_col, 'tgt_col': args.tgt_col, 'max_length': args.max_length}
        train, valid, test = map_records(train, tok_local, **kw), map_records(valid, tok_local, **kw), map_records(test, tok_local, **kw)
    else:
    ...
    for row in details:
        ...
        rows.append({'source': row['source'], 'reference': row['reference'], 'prediction': row['prediction'], 'reference_len': len(row['reference_tokens']), 'prediction_len': len(row['predicted_tokens']), 'token_f1': round(score, 4), 'length_gap': abs(len(row['reference_tokens']) - len(row['predicted_tokens']))})
    ...
    if details:
        save_attention(next(x for x in details if x['source'] == ok.iloc[0]['source']), outdir / 'attention_heatmap_best_case.png')
        save_attention(next(x for x in details if x['source'] == bad.iloc[0]['source']), outdir / 'attention_heatmap_failed_case.png')
